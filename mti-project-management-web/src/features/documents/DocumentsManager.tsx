                      ...
                      <div className="flex items-start gap-3 text-start">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-3 text-[11px] text-slate-400 mt-1 flex-wrap">
                            ...
                            <span>•</span>
                            <span>{formatDateCairo(doc.createdAt)}</span>
                          </div>
                        </div>
                      </div>
                ...
                {selectedDoc.versions.map((ver) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-[10px] text-slate-500">
                      <span>{ver.uploadedByUserName}</span>
                      <span>{formatDateCairo(ver.uploadedAt)}</span>
                    </div>
                    {ver.changeReason && (
              ...
              <div className="space-y-2 pt-2 border-t border-slate-700">
                ...
                <div className="space-y-1.5 max-h-36 overflow-y-auto">
                  {selectedDoc.approvals.map((app) => (
                    <div key={app.id} className="p-2 rounded-lg bg-slate-950/40 text-xs border border-slate-700/40">
                      <div className="flex items-center justify-between font-semibold">
                        <span className={app.status === 'Approved' ? 'text-emerald-400' : 'text-amber-300'}>
                          {app.status} (v{app.versionNumber})
                        </span>
                        <span className="text-[10px] text-slate-500">{formatDateCairo(app.createdAt)}</span>
                      </div>
                      {app.reason && <p className="text-[11px] text-slate-400 mt-1">{app.reason}</p>}
                    </div>
                  ))}
                </div>
              </div>
